...
import time
import os
from datasets.block import BlockDataset, LatentBlockDataset, GravitationalHackathonDataset
...
def load_hackathon():
    ...
    data = data['data']
    data = data.reshape(-1, 1, 2, 200)

    data = np.random.permutation(data)

    train = GravitationalHackathonDataset(data, train=True, transform=None)
    val = GravitationalHackathonDataset(data, train=False, transform=None)

    print("returning train val")
...
def load_data_and_data_loaders(dataset, batch_size):
    ...
    elif dataset == 'hackathon':
        training_data, validation_data = load_hackathon()
        training_loader, validation_loader = data_loaders(
            training_data, validation_data, batch_size)
